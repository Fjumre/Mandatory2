...
from socket import *
from datetime import datetime  # Bruges til at lave log-tidsstempler
...
# Print en besked i terminalen, så vi ved at serveren er klar
...
# Serveren stopper kun, hvis vi trykker Ctrl+C i terminalen
while True:
    ...
    try:
        ...
        # Hvis requesten er tom, print "-" som placeholder
        print("-")
    ...
    try:
        ...
        # Hvis det ikke er en GET-request eller linjen ikke har 3 dele, så lav fejl
        if len(parts) != 3 or parts[0] != "GET":
            raise ValueError("Bad Request")

        # Gem metode, sti og HTTP-version i separate variabler
        ...
        # Content-Type = fortæller browseren at det er HTML
        # "\n\n" = adskiller header fra body
        http_response = f"HTTP/1.1 200 OK\nContent-Type: text/html\n\n{response_body}"
        status_code = 200  # Gem statuskode til logging
    ...
    # Hvis requesten er forkert formateret (fx ikke GET)
    except Exception:
    ...
    try:
        request_line_log = request_text.splitlines()[0]  # Gem første linje af request til log
    except IndexError:
        request_line_log = "-"  # Hvis requesten er tom

    print(f"Request: {request_line_log} --> Status: {status_code}")

    # Lav log-linje som ligner Apache-format
    log_entry = f"{addr[0]} - - [{now}] \"{request_line_log}\" {status_code}\n"

    # Åbn logfilen i append-mode og skriv log-linje
